...
class Board {
    ...
    int redPieces() {
        return numPieces(PieceColor.RED);
    }

    /** Return number of blue pieces on the board. */
    int bluePieces() {
        return numPieces(PieceColor.BLUE);
    }
    ...
    void makeMove(Move move) {
        if (!legalMove(move)) {
            throw GameException.error("Illegal move: %s", move);
        }
        if (move.isPass()) {
            pass();
            return;
    ...
    void setBlock(char c, char r) {
        if (!legalBlock(c, r)) {
            throw GameException.error("illegal block placement");
        }
        // FIXME
        if (!canMove(PieceColor.RED) && !canMove(PieceColor.BLUE)) {
            _winner = PieceColor.EMPTY;
        }
    ...
    private int[] _numPieces = new int[PieceColor.BLUE.ordinal() + 1];
