...
class Game {
    ...
    int play() {
        ...
        _board.clear();
        setManual(PieceColor.RED);
        setAuto(PieceColor.BLUE);
        _exit = -1;
    ...
    void makeMove(String moveStr) {
        ...
        if (_board.legalMove(move)) {
            ...
            throw GameException.error("illegal move");
        }
        if (_verbose) {
            printBoard();
        }
    ...
    void block(String place) {
        if (_board.numMoves() > 0) {
            throw GameException.error("block-setting must precede first move.");
        }
        if (!place.matches("[a-i][1-9]")) {
            throw GameException.error("invalid square designation");
        }
    ...
    private void help() {
        Utils.printHelpResource(HELP, System.out);
    }
    ...
    private void executeCommand(String cmndStr) {
        ...
        try {
            ...
            case COMMENT:
                break;
            case AUTO:
                setAuto(PieceColor.parseColor(parts[0]));
                break;
            case BOARD:
                printBoard();
                break;
            ...
            case HELP:
                ...
                break;
            case MANUAL:
                setManual(PieceColor.parseColor(parts[0]));
                break;
            case NEW:
                clear();
                break;
            ...
            case PIECEMOVE:
                ...
                break;
            case ERROR:
                throw GameException.error("Unknown command.");
            default:
                break;
            }
